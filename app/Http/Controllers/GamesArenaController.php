...
class GamesArenaController extends Controller
{
    public function show(Request $request)
    {
        ...
        // v1.2 (May 2026): five-tier fallback so brand-new accounts
        // (no UserProgress rows yet) ALWAYS get a playable Arena deck.
        // Without this chain, a child who signs up and immediately
        // taps the Games Arena pin saw "🎮 No words yet!" because
        // the controller fell back to a single unit whose words
        // happened to lack media — and the deck builder needs at
        // least 4 words to produce all 11 round styles.

        if (empty($unlockedUnitIds)) {
            // Tier 1 fallback: use ALL units (not just U0) so a
            // brand-new account has the broadest possible word pool.
            $unlockedUnitIds = Unit::orderBy('unit_number')->pluck('id')->all();
        }

        $allUnitIds = Unit::pluck('id')->all();

        // Step A: words with media in unlocked units
        ...
        // Step B: any words in unlocked units (SVG fallback handles missing images)
        if ($words->count() < 4) {
            $words = Word::with(['audioTrack', 'unit:id,code,title,unit_number,color_key'])
                ->whereIn('unit_id', $unlockedUnitIds)
                ->get();
        ...
        // Step C: words with media across ALL units
        if ($words->count() < 4) {
            $words = Word::with(['audioTrack', 'unit:id,code,title,unit_number,color_key'])
        ...
        // Step D: any word in the database (last-ditch)
        if ($words->count() < 1) {
            $words = Word::with(['audioTrack', 'unit:id,code,title,unit_number,color_key'])->get();
        }

        \Illuminate\Support\Facades\Log::debug('Arena deck pool', [
            'user_id'         => $user->id,
