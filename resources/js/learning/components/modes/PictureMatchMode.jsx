...
/**
 * PictureMatchMode v3 — find identical pictures (no labels).
 *
 * Aimed at pre-readers. v3 fits a 720p tablet without scroll, uses
 * a 3-D flip animation, and shows the word via TTS when each card
 * is revealed.
 */
...
const PictureMatchMode = ({ lesson, deck = [], onComplete }) => {
        ...
        if (matched.size === totalPairs && totalPairs > 0) {
            const t = setTimeout(() => {
                const correctRounds = Array.from({ length: totalPairs }).map((_, i) => ({
                    roundId: `pic-correct-${i}`, correct: true,
                }));
                onComplete({
                    correct: totalPairs,
                    total: totalPairs,
    ...
    const colsClass = cards.length <= 8
    ...
    return (
        <div className="w-full max-w-3xl flex flex-col items-center gap-3 sm:gap-4 lg:gap-5 animate-fade-in-up px-2">
            <div className="w-full max-w-md bg-white/95 backdrop-blur rounded-2xl shadow-md border border-white px-4 py-2 flex flex-col gap-1.5">
                <p className="text-[10px] font-black text-pink-500 uppercase tracking-widest text-center">🎴 Picture Match · Find identical pairs</p>
                <div className="flex items-center gap-2">
                    <div className="flex-1 h-2 bg-pink-100 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-pink-400 to-rose-500" style={{ width: `${progressPct}%` }} />
                    </div>
                    <span className="text-[10px] font-black text-pink-600">{matched.size}/{totalPairs}</span>
                ...
                {cards.map((card) => {
                    ...
                    return (
                        <button
                            ...
                            className={`pic-card aspect-square rounded-xl sm:rounded-2xl transition-transform duration-300 ${isMatched ? "opacity-80 scale-95" : ""} ${
                            ...
                            <div className={`pic-inner ${isFlipped ? "is-flipped" : ""}`}>
                                <div className="pic-face pic-back">
                                    <div className="absolute inset-0 rounded-xl sm:rounded-2xl bg-gradient-to-br from-pink-400 via-rose-500 to-fuchsia-600 shadow-md border-2 border-white/30 flex items-center justify-center overflow-hidden">
                                        <span className="absolute -top-2 -left-2 w-12 h-12 bg-white/15 rounded-full" />
                                        <span className="absolute -bottom-2 -right-2 w-10 h-10 bg-white/10 rounded-full" />
                                        <span className="relative text-xl sm:text-3xl drop-shadow">🎴</span>
                                    </div>
                                </div>

                                <div className="pic-face pic-front">
                                    <div className={`absolute inset-0 rounded-xl sm:rounded-2xl border-2 sm:border-4 shadow-md flex items-center justify-center p-1.5 sm:p-2 ${
                                        isMatched ? "bg-emerald-50 border-emerald-300" : "bg-white border-pink-200"
                                    }`}>
            ...
            <style>{`
                ...
                .pic-card { perspective: 800px; background: transparent; border: none; padding: 0; }
                .pic-inner {
