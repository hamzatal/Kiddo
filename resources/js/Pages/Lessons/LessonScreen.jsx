        ...
        <div className="w-full max-w-4xl bg-white/95 backdrop-blur-2xl rounded-[3rem] shadow-[0_20px_60px_rgba(0,0,0,0.05)] border border-white flex flex-col md:flex-row items-center p-8 sm:p-14 gap-10 sm:gap-16 relative animate-fade-in-up">
            ...
            <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left w-full">
                ...
                <button
                    ...
                    ${isListening ? "bg-purple-300 text-white shadow-none translate-y-[6px] cursor-wait" : "bg-[#7C3AED] text-white shadow-[0_6px_0_#5B21B6] hover:bg-[#6D28D9] hover:translate-y-[2px] active:translate-y-[6px] active:shadow-none"}`}
                >
            ...
            <div className="bg-white/90 backdrop-blur-md px-10 py-6 rounded-[2.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.05)] border border-white mb-10 flex flex-col items-center gap-4">
                ...
                {gameMode === "audio-match" && (
                    <div className="flex flex-col items-center gap-3">
                        ...
                        <button
                            onClick={() =>
                                new Audio(wordData?.audio_path).play()
                            }
                ...
                {options.map((opt) => {
                    ...
                    return (
                        <button
                            ...
                            ${isWrong ? "border-red-200 bg-red-50 opacity-40 grayscale scale-95 cursor-not-allowed" : isWon ? "border-green-500 bg-green-50 scale-105 rotate-2 z-10" : "border-white hover:border-purple-300 hover:shadow-2xl hover:-translate-y-2"}`}
                        >
                            {opt.img ? (
                                <img
                                    src={opt.img}
                                    ...
                                    onError={(e) =>
                                        (e.target.outerHTML = `<span class="text-3xl font-black uppercase text-gray-400">${opt.text}</span>`)
                                    }
                                />
                            ) : (
...
const LessonScreen = ({ unit, wordData, progress }) => {
    ...
    const handleNext = () => {
        ...
        if (currentStep < 2) {
            // ننتقل للخطوة اللي بعدها في نفس الكلمة
            setCurrentStep(currentStep + 1);
            setIsStepCompleted(false);
        } else {
            // الطالب خلص اللعبة وأخذ الجائزة، هسا بنبعث للباك إند يحفظ التقدم ويجيب الكلمة الجديدة!
    ...
    return (
        <div className="h-[100dvh] w-screen bg-[#F4F8FB] font-sans flex flex-col overflow-hidden relative">
            ...
            <header className="absolute top-0 left-0 w-full p-5 sm:p-6 flex flex-col gap-4 z-50 pointer-events-none">
                ...
                {/* ── شريط التقدم للدرس (كم كلمة خلصنا) ── */}
                ...
                <div className="mx-auto flex items-center gap-2 bg-white/90 backdrop-blur-md px-3 sm:px-4 py-2 rounded-full shadow-lg border border-white pointer-events-auto">
                    {steps.map((s, i) => (
                        <div
                            key={i}
                            className={`flex items-center gap-2 px-3 py-1.5 rounded-full transition-all duration-500 ${currentStep === i ? `${s.bg} ${s.color} shadow-inner scale-105` : currentStep > i ? "bg-green-50 text-green-500" : "bg-transparent text-gray-400"}`}
                        >
            ...
            <footer className="absolute bottom-0 left-0 w-full p-5 sm:p-8 flex justify-between items-center z-50 pointer-events-none">
                ...
                >
                    <span className="text-lg leading-none">←</span> Back to Map
                </button>

                <button
                    onClick={handleNext}
                    disabled={!isStepCompleted}
                    className={`pointer-events-auto px-10 sm:px-14 py-4 sm:py-5 rounded-[2rem] font-black text-sm sm:text-lg transition-all duration-300 shadow-lg flex items-center gap-3 ${isStepCompleted ? "bg-[#10B981] text-white shadow-[0_8px_0_#059669] hover:translate-y-[2px] active:translate-y-[8px] active:shadow-none" : "bg-gray-200 text-gray-400 cursor-not-allowed shadow-[0_8px_0_#D1D5DB]"}`}
                >
                    CONTINUE ➔
                </button>
            </footer>
